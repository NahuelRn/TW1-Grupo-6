...
public class ServicioCalculoRecompensaImpl implements ServicioCalculoRecompensa {
    ...
    @Autowired
    public ServicioCalculoRecompensaImpl(RepositorioConfiguracionJuego repositorioConfiguracionJuego) {
        this.REPOSITORIO_CONFIGURACION_JUEGO = repositorioConfiguracionJuego;
    }

    @Override
    public RecompensaDTO obtenerRecompensa(Partida partida) {
        Integer experienciaBase = this.REPOSITORIO_CONFIGURACION_JUEGO.obtenerValor(ConfiguracionJuego.EXPERIENCIA_BASE);

        RecompensaDTO recompensaDTO = new RecompensaDTO();

        int hpEnemigo = obtenerHpEnemigo(partida);
        if (hpEnemigo <= 0) {
            Integer oroBase = this.REPOSITORIO_CONFIGURACION_JUEGO.obtenerValor(ConfiguracionJuego.ORO_BASE);
    ...
    private void calcularRecompensaVictoria(Partida partida, Integer oroBase, Integer experienciaBase, RecompensaDTO recompensaDTO) {
        Integer oro = oroBase;
