...
public class ServicioTiendaImpl implements ServicioTienda {
    ...
    public ServicioTiendaImpl(RepositorioUsuario repositorioUsuario, RepositorioCarta repositorioCarta, RepositorioInventario repositorioInventario) {
    ...
    public void comprarCarta(Usuario usuario, Long identificadorCarta) {
        ...
        if (usuario.getNivel() < carta.getNivelDesbloqueo()) {
            throw new RuntimeException("Error, no tienes el nivel necesario para desbloquear esta carta.");
        }
        ...
        ItemInventario itemInventario = this.REPOSITORIO_INVENTARIO.buscarItemDeJugador(usuario.getId(), carta.getId());
